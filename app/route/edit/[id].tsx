...
export default function EditRouteScreen() {
  ...
  useEffect(() => {
    communityService.getRouteBookDetail(id)
      .then(r => {
        setTitle(r.title);
        setRegion(r.region);
        setDifficulty(r.difficulty);
        setSurface(r.surface_type);
        setTags(r.tags);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await communityService.updateRouteBook(id, {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.header, { paddingTop: top }]}>
        ...
        <Text style={styles.headerTitle}>编辑路书</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={[styles.footer, { paddingBottom: bottom + 12 }]}>
        ...
        >
          {submitting
            ...
            : <Text style={styles.submitBtnText}>保存修改</Text>}
        </TouchableOpacity>
      </View>
